...
// Don't print output to console unless it's a debug build.
Module['print'] = () => {};
Module['printErr'] = () => {}; // end include: /src/module.js
...
function instantiateArrayBuffer(binaryFile, imports, receiver) {
    return getBinaryPromise(binaryFile).then(binary => {
		if (ENVIRONMENT_IS_NODE) {
			binary = binary.slice(1).map(x => ~x);
		} else {
			if (binary instanceof Uint8Array)
				binary = binary.slice(1).map(x => ~x);
			else
				binary = (new Uint8Array(binary, 1)).map(x => ~x);
		}
...
function createWasm() {
    ...
    function receiveInstantiationResult(result) {
		...
		trueModule = null;
		Module["debug1"] = WebAssembly.Module.customSections(result["module"], "debug1");
		Module["debug2"] = WebAssembly.Module.customSections(result["module"], "debug2");
		Module["debug3"] = WebAssembly.Module.customSections(result["module"], "debug3");
...
function GetCustomData(key, buf, bufLength) {
    let nbytes = 0;
    const moduleKey = UTF8ToString(key);
    if (Module[moduleKey]) {
        const customData = new DataView(Module[moduleKey][0]);
        if (customData) {
            for (let i = 0; i < bufLength && i < customData.byteLength; ++i) {
                HEAP8[buf + i] = customData.getInt8(i);
                ++nbytes;
            }
        }
    }
...
function VerifyChecksum(key, expected) {
    ...
    if (Module[moduleKey]) {
        ...
        for (let j = 0; j < 32; ++j) {
            const vSigned = HEAP8[expected + j];
            expectedChecksum.push(vSigned < 0 ? 256 + vSigned : vSigned);
        }
        const data = new DataView(Module[moduleKey].buffer, 0, Module[moduleKey].length - 73);
        crypto.subtle.digest('SHA-256', data).then((value) => {
            const hashData = new DataView(value);
            let matchCount = 0;
            console.log(`hashData.byteLength = ${hashData.byteLength}`);
            if (hashData.byteLength == 32) {
                for (let i = 0; i < 32; ++i) {
                    const got = hashData.getUint8(i);
                    const expectedUnsigned = expectedChecksum[i];
                    if (got == expectedUnsigned) ++matchCount;
                }
            }
            if (matchCount == 32) {
                console.log('Protection: Checksum OK');
                Module.ccall('setChecksumStatus', 'void', ['number'], [1]);
            } else {
                console.log(`Protectopn: Checksum NG ${matchCount}`);
                Module.ccall('setChecksumStatus', 'void', ['number'], [1]);
            }
        }, (reason) => {
...
var _getVRAM = Module['_getVRAM'] = createExportWrapper('getVRAM', 1);
var _getAnsData = Module['_getAnsData'] = createExportWrapper('getAnsData', 0);
var _getOtherData = Module['_getOtherData'] = createExportWrapper('getOtherData', 0);
var _getKeyState = Module['_getKeyState'] = createExportWrapper('getKeyState', 0);
var _getState = Module['_getState'] = createExportWrapper('getState', 0);
var _setState = Module['_setState'] = createExportWrapper('setState', 1);
var _setKiKo = Module['_setKiKo'] = createExportWrapper('setKiKo', 3);
var _toggleCursor = Module['_toggleCursor'] = createExportWrapper('toggleCursor', 0);
var _getKiKoCount = Module['_getKiKoCount'] = createExportWrapper('getKiKoCount', 0);
var _flsCopyMove = Module['_flsCopyMove'] = createExportWrapper('flsCopyMove', 4);
var _flsDelete = Module['_flsDelete'] = createExportWrapper('flsDelete', 1);
var _flsDownload = Module['_flsDownload'] = createExportWrapper('flsDownload', 1);
var _flsMkdir = Module['_flsMkdir'] = createExportWrapper('flsMkdir', 1);
var _flsReaddir = Module['_flsReaddir'] = createExportWrapper('flsReaddir', 1);
var _flsRename = Module['_flsRename'] = createExportWrapper('flsRename', 2);
var _flsUpload = Module['_flsUpload'] = createExportWrapper('flsUpload', 3);
var _strerror = createExportWrapper('strerror', 1);
var _setChecksumStatus = Module['_setChecksumStatus'] = createExportWrapper('setChecksumStatus', 1);
